...
namespace Crystal
{
    public abstract class SafeAreaBase : MonoBehaviour
    {
        ...
        private static bool _isSafeAreaCalculated;
        [SerializeField] protected bool ConformX = true;  // Conform to screen safe area on X-axis (default true, disable to ignore)
        [SerializeField] protected bool ConformY = true;  // Conform to screen safe area on Y-axis (default true, disable to ignore)
        [SerializeField] protected bool Logging = false;  // Conform to screen safe area on Y-axis (default true, disable to ignore)
        [SerializeField] protected bool _ignoreTopSafeArea = false;
        ...
        protected const int UpdateFramesCount = 60;
        protected int CurrentFramesCount;
        void Update()
        {
        ...
        Rect GetSafeArea()
        {
            if (!_isSafeAreaCalculated)
            {
                CalculateNativeSafeArea();
            }
            Rect safeArea = GetCalculatedSafeArea();
        ...
        private Rect GetCalculatedSafeArea()
        {
            return Screen.safeArea;
        }

        private void CalculateNativeSafeArea()
        {
            _isSafeAreaCalculated = true;
        }
    }
}
