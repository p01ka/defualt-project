...
public class NativeShare
{
    ...
    [System.Runtime.InteropServices.DllImport("__Internal")]
    private static extern int _Is_Instagramm_Installed();

    [System.Runtime.InteropServices.DllImport("__Internal")]
    private static extern void _Share_To_Instagram(string filePath, string text);

    [System.Runtime.InteropServices.DllImport("__Internal")]
    private static extern int _Is_Facebook_Installed();

    // [System.Runtime.InteropServices.DllImport("__Internal")]
    // private static extern void _Share_To_Facebook(string filePath, string text);
#endif
    ...
    public static bool IsInstagramInstalled()
    {
        ...
        return false;
#elif UNITY_ANDROID
        return TargetExists(InstagramPackageName);
#elif !UNITY_EDITOR && UNITY_IOS
        int result = _Is_Instagramm_Installed();
        return result == 1;
#endif
    }
    ...
    public static bool IsFacebookInstalled()
    {
        ...
        return false;
#elif UNITY_ANDROID
        return TargetExists(FacebookPackageName);
#elif !UNITY_EDITOR && UNITY_IOS
        int result = _Is_Facebook_Installed();
        return result == 1;
#endif
    }

    public static void ShareToInstagram(string filePath, string text)
    {
#if UNITY_ANDROID
        new NativeShare().AddFile(filePath).SetText(text).SetTarget(InstagramPackageName).Share();
#elif !UNITY_EDITOR && UNITY_IOS
        _Share_To_Instagram(filePath,text);
#endif
    }

    public static void ShareToFacebook(string filePath, string text)
    {
#if UNITY_ANDROID
        new NativeShare().AddFile(filePath).SetText(text).SetTarget(FacebookPackageName).Share();
#elif !UNITY_EDITOR && UNITY_IOS
