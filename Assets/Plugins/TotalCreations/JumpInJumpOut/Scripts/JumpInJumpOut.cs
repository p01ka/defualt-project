...
namespace TotalCreations
{
    namespace UI
    {
        ...
        public class JumpInJumpOut : MonoBehaviour
        {
            ...
            public void InitWithCanvasGroup(CanvasGroup canvasGroup = null)
            {
                if (canvasGroup != null)
            ...
            ///</summary>
            public void JumpIn(Action jumpInDelegate = null)
            {
                Show(jumpInDelegate);
            }
            ...
            ///</summary>
            public void JumpOut(Action jumpOutDelegate = null)
            {
                Hide(jumpOutDelegate);
            }
            ...
            public IEnumerator ShowRoutine(Action onFinished = null)
            {
                if (mState == State.Invisible)
                {
                    ...
                    if (blendActive)
                    {
                        ...
                        blendTween = CanvasGroup.DOFade(1f, showDuration).SetEase(showBlendEase).SetUpdate(true);
                    }
                    else
                    {
                    ...
                    if (rotationActive)
                    {
                        this.transform.localRotation =
                            Quaternion.Euler(rotationAxis * showStartMinMaxRotation.Random());

                        Vector3 endValue = rotationAxis * showMinMaxRotation.Random();
                        rotationTween = this.transform.DOLocalRotate(endValue, showDuration, RotateMode.Fast)
                            .SetEase(showRotationEase).SetUpdate(true);
                    }
                ...
                if (mState == State.Visible)
                {
                    ...
                    if (shakeActive && hideDuration > 0f)
                        shakeTween = this.transform
                            ...
                            .SetEase(hideShakeEase).SetUpdate(true);
                    if (scaleActive)
                    {
                    ...
                    if (blendActive)
                    {
                        blendTween = CanvasGroup.DOFade(0f, hideDuration).SetEase(hideBlendEase).SetUpdate(true);
                    }

                    if (rotationActive)
                    {
                        Vector3 endValue = rotationAxis * hideMinMaxRotation.Random();
                        rotationTween = this.transform.DOLocalRotate(endValue, hideDuration, RotateMode.Fast)
                            .SetEase(hideRotationEase).SetUpdate(true);
                    }

                    yield return new WaitForSecondsRealtime(hideDuration);
                    if ((shakeActive && (this.transform.localScale.x == 0f || this.transform.localScale.y == 0f ||
                                         this.transform.localScale.z == 0f)) || blendActive)
                    {
            ...
            public void SetStateImmediately(State state)
            {
                mState = state;
            }
        }
